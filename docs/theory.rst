...
The FEM solver discretises the 2-D transient heat equation:
...
on a rectangular domain using **bilinear quadrilateral (Q4)** elements with
2 × 2 Gauss quadrature.

Element matrices
~~~~~~~~~~~~~~~~

For each Q4 element, the stiffness and mass matrices are:

.. math::

   K_e = \int_{\Omega_e} \lambda\,(\nabla N)^T (\nabla N)\,d\Omega

.. math::

   M_e = \int_{\Omega_e} \rho c_p\,N^T N\,d\Omega

where *N* are the bilinear shape functions.  These are evaluated numerically
using 2 × 2 Gauss quadrature.

Graded mesh
~~~~~~~~~~~

The mesh uses a grading function that provides fine spacing near cable
locations and coarser spacing in the far field, improving accuracy near the
heat sources without excessive computational cost.

Boundary conditions
~~~~~~~~~~~~~~~~~~~

- **Top and bottom:** Dirichlet (prescribed temperature from the ground model)
- **Left and right:** zero-flux Neumann (undisturbed far field)

Time integration
~~~~~~~~~~~~~~~~

Transient problems are solved with **implicit Euler** using SciPy's sparse
