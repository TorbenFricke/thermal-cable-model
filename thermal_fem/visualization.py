...
from thermal_fem.solver import TransientResult
...
def plot_soil_temperature_field(
    fem_result: np.ndarray,
    x_coords: np.ndarray,
    y_coords: np.ndarray,
    ...
    """Contour plot of the 2-D soil temperature field from the FEM solver.

    Parameters
    ----------
    fem_result : ndarray, shape (ny, nx)
        Temperature field on the regular grid.
    x_coords, y_coords : ndarray
        1-D coordinate arrays (these are the mesh node coordinates).
    cable_positions : list of (x, depth) tuples, optional
        Mark cable locations on the plot.
    show_mesh : bool
        If True, overlay the FEM mesh grid lines.
    mesh_color, mesh_alpha, mesh_linewidth
        Styling for the mesh overlay.
    """
    fig, ax = plt.subplots(1, 1, figsize=figsize)
    X, Y = np.meshgrid(x_coords, y_coords)
    cs = ax.contourf(X, Y, fem_result, levels=30, cmap=cmap)
    fig.colorbar(cs, ax=ax, label="Temperature [°C]")
    ax.contour(X, Y, fem_result, levels=15, colors="k", linewidths=0.3, alpha=0.5)

    if show_mesh:
        for xv in x_coords:
            ax.axvline(xv, color=mesh_color, linewidth=mesh_linewidth,
                       alpha=mesh_alpha)
        for yv in y_coords:
            ax.axhline(yv, color=mesh_color, linewidth=mesh_linewidth,
                       alpha=mesh_alpha)

    if cable_positions:
        for x, d in cable_positions:
            ax.plot(x, d, "wo", markersize=8, markeredgecolor="black")

    ax.set_xlabel("Horizontal position [m]")
    ax.set_ylabel("Depth [m]")
    ax.set_title("Soil Temperature Field")
    ax.invert_yaxis()
    fig.tight_layout()
    return fig
